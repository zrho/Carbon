...
void memory_free(uintptr_t phys) {
	// Allocate buffer
	memory_free_req_t *req = (memory_free_req_t *) ipc_buffer_size(
			IPC_BUFFER_SEND, sizeof(memory_free_req_t));

	// Fill request
	req->header.api_low = MEMORY_API_LOW;
	req->header.api_high = MEMORY_API_HIGH;
	req->header.procedure = MEMORY_PROC_FREE;

	req->physical = phys;

	// Send message
	ipc_send(
			sizeof(memory_free_req_t),
